...
# 화면 크기 설정
screen_width = 640 # 프레임의 가로 크기
screen_height = 480 # 프레임의 세로 크기
...
# 게임 종료 메시지 / TimeOut(시간 초과), Mission Complete(성공), Game over(캐릭터가 공에 맞음)
...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        # 공이 가로 벽에 닿았을때 이동 위치 변경(튕겨 나오는 효과)
        if ball_pos_x < 0 or ball_pos_x > screen_width - ball_width:
            ball_val["to_x"] = ball_val["to_x"] * -1
        
        # 공이 세로 경계선(천장, 스테이지)에 닿았을때
        # 스테이지에 튕겨서 올라가는 처리
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                weapon_to_remove = weapon_idx # 해당 무기를 없애기 위한 값 정보
